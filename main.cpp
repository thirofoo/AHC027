...
struct Rect{
    ...
    explicit Rect(int _x, int _y, int _xl, int _yl) : x(_x), y(_y), xl(_xl), yl(_yl) {
        area = xl*yl;
    }
    ...
    void paintRect(int idx, int tx, int ty) {
        ...
        while( true ) {
            ...
            rep(dir,DIR_NUM) {
                ...
                // ちょっと寄り道して多いところを掃除
                if( (ans.size()-clean_turn[nx][ny])*d[nx][ny] >= 2*d_total ) {
                    clean_turn[tx][ty] = ans.size();
                    ans.emplace_back(pair(nx,ny));
                    clean_turn[nx][ny] = ans.size();
                    ans.emplace_back(pair(tx,ty));
                }
            }
